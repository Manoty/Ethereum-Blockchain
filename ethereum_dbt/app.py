...
st.markdown("Explore daily metrics and trends for selected cryptocurrencies.")
st.subheader("Summary Metrics")
st.write("Total Records:", len(df))
st.write("Average Daily Return:", f"{df['daily_return'].mean():.6%}")
...
st.markdown("_Shows how the daily returns of selected assets fluctuate over the chosen period._")
fig_return = px.line(
    df,
    x="date",
    y="daily_return",
    ...
    hover_data={"daily_return": ":.2%", "date": "|%Y-%m-%d"}
)
...
st.markdown("_Displays a smoothed trend of daily returns to reduce short-term noise._")
fig_ma = px.line(
    df,
    x="date",
    ...
    hover_data={"daily_return_7d_ma": ":.2%", "date": "|%Y-%m-%d"}
)
...
st.markdown("_Shows continuous compounded returns, useful for financial modeling._")
fig_log = px.line(
    df,
    x="date",
    y="log_return",
    ...
    hover_data={"log_return": ":.2%", "date": "|%Y-%m-%d"}
)
...
st.markdown("_Displays the trading volume to observe liquidity and market activity._")
fig_volume = px.line(
    df,
    x="date",
    y="volume",
...
st.markdown("_Compare selected metrics across assets. Volume automatically uses the secondary Y-axis._")
...
if metrics:
    ...
    for metric in metrics:
        for asset in df['asset'].unique():
            ...
            hover_fmt = ",.0f" if metric == "volume" else ":.2%"
            if metric == "volume":
                fig_multi.add_trace(
                    go.Scatter(
                        x=df_asset['date'],
                        ...
                        mode='lines',
                        name=f"{asset} - {metric}",
                        hovertemplate=f"%{{y:{hover_fmt}}}<br>Date: %{{x|%Y-%m-%d}}"
                    ),
                    secondary_y=True
                )
            else:
